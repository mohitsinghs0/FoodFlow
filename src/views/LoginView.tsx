...
export const LoginView: React.FC = () => {
  ...
  const { login, signInWithGoogle, loginAsDemoCustomer, loginAsDemoOwner } = useAuth();
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (res.isNewUser || !res.user.profileCompleted) {
        navigate(res.user.role === 'owner' ? '/setup-shop' : '/complete-profile');
      } else {
    ...
    <div className="min-h-[80vh] flex flex-col justify-center px-4 py-8 sm:px-6 lg:px-8 max-w-md mx-auto animate-in fade-in duration-200">
      ...
      <div className="bg-white rounded-3xl border border-slate-200/90 p-6 sm:p-7 shadow-sm space-y-5">
        ...
        <button
          ...
          disabled={loading}
          className="w-full py-2.5 px-4 rounded-2xl border border-slate-200 bg-white hover:bg-slate-50 active:scale-[0.99] text-slate-700 font-bold text-xs shadow-xs transition-all flex items-center justify-center gap-2.5 cursor-pointer disabled:opacity-50"
        >
